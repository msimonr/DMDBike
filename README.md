...
¿Como promover la actividad fisica en un evento masivo? ¿Como hacerlo si se dispone de poco espacio y un tiempo limitado por cada participante? ¿Como hacer que sea divertido y desafiante?
...
- Raspberry Pi 4 con RaspbianOS.
- TV/monitor conectado por HDMI donde se muestra el panel principal.

### Software
- Python 3 + Flask como servidor web.
- SQLite (`bici.db`) como base de datos sencilla y eficiente para el caso de usos.
...
- `WHEEL_CIRCUMFERENCE_M`: circunferencia real de la rueda.
...
- (/) Muestra la pantalla principal. En el centro se ve el circulo del Día Mundial de la Diabetes junto con los kilometros totales acumulados, la distancia recorrida en la sesión actual con actualizaciones en tiempo real. A la izquierda se visualiza el top de sesiones con nombre, distancia y fecha. En la derecha se encuentra una galeria donde se muestran al azar los distintos participantes con su nombre, foto y distancia.
...
- (/carga_manual) Esta ruta permite guardar una sesión ingresando los km manualmente, la distancia aqui ingresada cuenta para el Top pero no se suma a la distancia total desplegada en la pantalla principal. Esto debe utilizarse para casos puntuales donde se haya olvidado registrar al participante cuando correspondia.s
- (/sync) Dado que la RaspberryPi no contara con conexión a internet, la fecha y hora estara desincronizada con la real. Al comenzar el evento, este endpoint permite sincronizar la fecha y hora con la del celular utilizado pará operar los paneles de administración.
...
En todo momento se asume que se esta utilizando una RaspberryPi 4 con sistema operativo RaspbianOS.
...
Para este proyecto se utilizó un sensor Hall A3144, que es un dispositivo de
colector abierto. Esto significa que su salida **no genera voltaje por sí
misma**, sino que necesita una resistencia pull-up para obtener un nivel lógico.
...
⚠️ Nunca conectar la salida del A3144 directamente a 5 V si va a un GPIO de la
Raspberry Pi. Los pines GPIO solo toleran hasta **3.3 V**, un valor mayor podría dañar la placa. 
...
Es recomendable configurar la raspberry para que inicialize app.py y sensor.py automaticamente, ademas de lanzar chromium en modo kiosk para la pantalla principal.

Revisar las rutas que se encuentran en estos archivos para que coincidan con las que correspondan.
...
User=mauripi
Group=mauripi
WorkingDirectory=/home/mauripi
Environment=DISPLAY=:0
Environment=XAUTHORITY=/home/mauripi/.Xauthority
...
Iniciar el daemon, esto hara que los scripts inicien apenas la raspberry encienda y ademas los reiniciara en caso de que fallen.
...
- El sistema deberia arrancar solo para ser operado sin necesidad de mouse o teclado.
- La hora debe sincronizarse al inicio del encendido, ya que sin conectarse a internet la raspberry tendra la fecha y hora del ultimo instante en el que estaba encendida.
- Las fotos se comprimen en WEBP para optimizar el tamaño, aun así, es recomendable utilizar una micro sd con tamaño suficiente para evitar inconvenientes, para este proyecto se utilizo una micro sd de 128 Gb.
...
- Es recomendable cuando se sube un participante reiniciar la sesión, para evitar que esta se reinicie automaticamente mientras el participante esta activo.
...
De esta manera es posible recuperar la información de las sesiones. Es recomendable luego de cada evento realizar un backup y almacenarlo en un sitio externo a la PI.

## Créditos

Proyecto desarrolado en el marco del Día Mundial de la Diabetes 2025.
...
Este proyecto se distribuye bajom la licencia MIT.
Puedes usarlo, modificarlo y compartirlo libremente, siempre manteniendo el crédito original.

MIT © 2025 Mauricio Simón Roglias
