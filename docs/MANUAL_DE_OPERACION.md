...
3. Verificar que en la pantalla aparezca la foto del participante, cuando aparezca refrescar la página si los datos no se limpiaron automáticamente.

### Carga manual (si es necesario)
- URL: `http://IP:5000/pictures_manual`
- Registrar nombre + km + foto.
    - Los km ingresados aquí no contabilizan para el total de km acumulados.

## Problemas comunes
- **La pantalla no actualiza:** recargar TV o reiniciar Raspberry.
- **Hora incorrecta:** volver a `/sync`.
- **La distancia del participante no aumenta:** Verificar que el sensor no se haya movido y esté a la distancia correcta.
